...
def safe_get_episode(season, episode):
    ...
    # Fix episodes where episode number incorrectly has season prefix
    if season >= 10 and ep_str.startswith(season_str):
        fixed_str = ep_str[len(season_str):]  # Remove season prefix
        fixed_str = fixed_str.lstrip('0')     # Strip leading zeros
        if fixed_str.isdigit() and len(fixed_str) > 0:
            fixed_ep = int(fixed_str)
            print(f"Corrected episode number for S{season}: Ep.{episode} → Ep.{fixed_ep}")
            episode = fixed_ep
